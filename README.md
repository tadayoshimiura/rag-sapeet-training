...
## Minimal Ops (RAG System)

See `docs/minimum_ops.md` for:
- prompt rendering tests
- mocked integration tests
- Bedrock question generation command

See `docs/aurora_mapping.md` for the Aurora mapping checklist.
See `docs/repo_setup.md` for fork/rename and secret safety.

## How It Works (Plain English)

- **System Prompt**: global rules for the model.
  - `configs/system_prompts.json` (`question_gen_prompt`, `summary_gen_prompt`, `router_prompt`)
- **User Prompt**: the actual inputs/variables for a run.
  - question gen: `pipelines/question_gen/prompts.py`
  - summary: `scripts/entry/run_summary_gen.py`
- **Router**: decides which prompt to use from a user request.
  - `scripts/entry/run_llm_pipeline.py` uses `router_prompt` and a keyword fallback

## Model Defaults

Defaults are in `configs/llm_pipeline_settings.json`:
- question_gen model: `anthropic.claude-3-5-sonnet-20240620-v1:0`
- summary_gen model: `anthropic.claude-3-5-sonnet-20240620-v1:0`
- region: `ap-northeast-1`

## Environment Safety

- Secrets go in `.env` (never commit).
- `.env` and `outputs/`, `logs/`, `reports/` are ignored by `.gitignore`.
- `.env.example` is the public template.

## Batch (Optional)

For manual batch runs, loop the command in `docs/minimum_ops.md`.
If SQS is introduced, the queue payload can contain:
- `lecture_id`, `test_genre`, `question_count`
and a worker can call `scripts/entry/run_question_gen.py` per job.
